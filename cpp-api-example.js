...
let first = true
let stop = false
window.addEventListener('click', () => stop = !stop)
  ...
  .then(results => {
    ...
    scriptNode.onaudioprocess = (audioProcessingEvent) => {
      ...
      try {
        ...
        for (let i = 0; i < outputFloats.Size(); i += 2) {
          outputDataLeft[i] = outputFloats.Get(i)
          outputDataLeft[i + 1] = outputFloats.Get(i + 1)
        }
      }
      catch (ex) {
